...
# read gtf file which tells us chromosome & location for a gene
...
def annotate_df(norm_df, annotations):
    gene_list = norm_df.index
    gene_list = [gene.split('.')[0] for gene in gene_list]
    chroms = [annotations[gene][0] for gene in gene_list if gene in annotations]
    starts = [annotations[gene][1] for gene in gene_list if gene in annotations]
    ends = [annotations[gene][2] for gene in gene_list if gene in annotations]
    strands = [annotations[gene][3] for gene in gene_list if gene in annotations]
    ...
    norm_df.insert(4, 'gid', norm_df.index, allow_duplicates=True)
...
if __name__ == '__main__':
    ...
    normalized_df = process_gct(args.gct_tpm, args.gct_reads, args.frac_thresh,
                                                args.read_thresh, args.tpm_thresh)
    annotated_df = annotate_df(normalized_df, gtf_annotations)
