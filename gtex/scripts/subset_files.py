...
def parseargs():    # handle user arguments
    ...
    parser.add_argument('--gene_gct', required=True, 
        help='Gene-level gct file for a specific tissue (TPM or read counts ok). Required.')
    parser.add_argument('--iso_gct_tpm', required=True, 
        help='Isoform-level TPM gct file. Required.')
    parser.add_argument('--iso_gct_reads', required=True, 
        help='Isoform-level read counts gct file. Required.')
...
def read_gct_gene_file(gct_gene_fname):
...
def subset_gct(basename, gct_fname, intersect_samples, tpm_or_reads='tpm'):
    ...
    with(gzip.open(gct_fname, 'r')) as infile:
        with(gzip.open(basename + '.' + tpm_or_reads + '.gct.gz', 'w')) as outfile:
            ...
            cols_to_keep += [i for i in range(2, len(header)) if header[i] in intersect_samples]
...
if __name__ == '__main__':
    args = parseargs()
    gct_samp, id2samp = read_gct_gene_file(args.gene_gct)
    ...
    subset_gct(args.output, args.iso_gct_tpm, intersect_samp, tpm_or_reads='tpm')
    subset_gct(args.output, args.iso_gct_reads, intersect_samp, tpm_or_reads='reads')
