...
h2noncis=(0.0 0.05 0.1 0.2 0.4)

index=$((${SGE_TASK_ID}-1))
h2c_index=$((${index}%${#h2cis[@]}))
h2n_index=$((${index}/${#h2cis[@]}))
this_h2c=${h2cis[$h2c_index]}
this_h2n=${h2noncis[$h2n_index]}
outdir=results/mvn_genomewide_numiso_4_h2noncis/h2cis_${this_h2c}_h2noncis_${this_h2n}
[ ! -d ${outdir} ] && mkdir ${outdir}  # make outdir if it doesn't exist

for i in {1..1} #10}
do
    python scripts/mvn_simulation.py --vcf data/genotypes_yri.vcf.gz --pheno data/qqnorm_all.bed.gz --dropout_rate 0.5 --dropout_iso 0.0 --h2cis ${this_h2c} --h2noncis ${this_h2n} --neg_pct 0.5 --max_corr 0.49 --min_corr 0.09 --max_corr_env 0.49 --min_corr_env 0.09 --output ${outdir}/sim_phenos_${i} --window 50000 --num_iso 4 # --num_causal 3 --num_iso 2
