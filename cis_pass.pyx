...
def nominal_pass(vcf, tx2info, tx2expr, gene_to_tx, meta_lines, window, bcftools, n_perms, nominal_thresh, methods):
    ...
    for gene in gene_to_tx:
        ...
        if n_perms > 0:
            ...
            snp, stat, pval, perm_peak_pvals = find_peak_snp(tx2expr, snp2geno, txlist, nominal_thresh, permuted_tx2expr, n_perms, methods)
            for m in methods:
            ...
            snp, stat, pval, perm_peak_pvals = find_peak_snp(tx2expr, snp2geno, txlist, nominal_thresh, {}, n_perms, methods)
        for m in methods:
