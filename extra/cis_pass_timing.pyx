...
def find_peak_snp(tx2expr, snp2geno, txlist, nominal_thresh, permuted_tx2expr, n_perms, methods):
    ...
    for m in methods:
        ...
        elapsed[m] = 0.0
    elapsed['regressions'] = 0.0
    if len(snp2geno) == 0:
        return peak_snps, peak_stats, peak_pvals, perm_peak_pvals, elapsed
    ...
    for snp in snp2geno:
        ...
        elapsed['regressions'] += (t_after - t_before).total_seconds()
        for m in methods:
            if len(txlist) != 1:
                t_before = datetime.datetime.now()
                if m == 'wilks':
                ...
                t_after = datetime.datetime.now()
                elapsed[m] += (t_after - t_before).total_seconds()
...
def nominal_pass(vcf, tx2info, tx2expr, gene_to_tx, meta_lines, window, bcftools, n_perms, nominal_thresh, methods):
    ...
    for gene in gene_to_tx:
        ...
        if n_perms > 0:
            ...
            snp, stat, pval, perm_peak_pvals, t_elapsed = find_peak_snp(tx2expr, snp2geno, txlist, nominal_thresh, permuted_tx2expr, n_perms, methods)
            for m in methods:
                if pval[m] < nominal_thresh:
                    ...
                    perm_pvals[m][gene] = [dir_perm_pval, beta_perm_pval]
        else:
            snp, stat, pval, perm_peak_pvals, t_elapsed = find_peak_snp(tx2expr, snp2geno, txlist, nominal_thresh, {}, n_perms, methods)
        for m in t_elapsed:
            total_times[m] += t_elapsed[m]
        for m in methods:
            gene_results[m][gene] = [snp[m], stat[m], pval[m]]
    fnull.close()
    print(total_times)
